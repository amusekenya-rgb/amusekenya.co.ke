-- Insert missing camp forms with correct slugs
-- This ensures all 6 camp forms exist in the database
...
('Easter Camp Form Config', 'easter-form', 'camp_form', 'published', 'Easter camp registration form configuration', jsonb_build_object(
  'formConfig', jsonb_build_object(
    'pricing', jsonb_build_object('halfDayRate', 1500, 'fullDayRate', 2500, 'currency', 'KES'),
    ...
    'buttons', jsonb_build_object('registerOnly', 'Register Only', 'registerAndPay', 'Register & Pay Now', 'addChild', 'Add Another Child', 'removeChild', 'Remove'),
    'messages', jsonb_build_object('registrationSuccess', 'Registration submitted successfully! We''ll contact you shortly.', 'registrationError', 'Failed to submit registration. Please try again.', 'chooseOption', 'Choose your registration option:', 'paymentComingSoon', 'Payment integration coming soon. Both options will complete your registration.'),
    'specialNeedsSection', jsonb_build_object('title', 'Special Needs & Medical Information', 'description', 'Please provide any information about allergies, medical conditions, or special accommodations needed.')
  )
)),
